...
export const Login = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    Validate password
    const hashedPassword = authentification(
      user.authentification.salt,
      password
    );

    // const validPassword = isPasswordValid(password,  user.authentification.password )
    ...
    // Set session cookie
    res.cookie("TEST-AUTH", user.authentification.sessionToken, {
      domain: "localhost",
      path: "/",
    });
